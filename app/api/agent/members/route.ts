 ...
 *   ?moderators_only=true   — only moderators (chat-moderation role)
 *   ?admins_only=true       — only admins (full access)
...
export async function GET(request: NextRequest) {
  ...
  const adminsOnly     = searchParams.get('admins_only') === 'true'
 ...
 * Body: { name, email, is_moderator?, is_admin?, display_name?, access_badges? }
 */
export async function POST(request: NextRequest) {
  if (!verifyAgentKey(request)) return agentUnauthorized()

  const { name, email, is_moderator, is_admin, display_name, access_badges } = await request.json()
...
export async function PATCH(request: NextRequest) {
  ...
  const allowed = ['display_name', 'is_moderator', 'is_admin', 'is_banned', 'name']
