...
| POST | `/members` | `{ name, email, is_moderator?, is_admin?, display_name?, access_badges? }` | Add (upserts on email). Returns login credentials. Badges: `vip_member`, `private_student`. |
